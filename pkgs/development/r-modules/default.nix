...
let
  ...
  packagesWithBuildInputs = {
    ...
    ACTCD = [pkgs.libiconv];
    ADMMsigma = [pkgs.libiconv];
    AMIAS = [pkgs.libiconv];
    ANN2 = [pkgs.libiconv];
    APIS = [pkgs.libiconv];
    BsMD = [pkgs.libiconv];
    DescTools = [pkgs.libiconv];
    GDINA = [pkgs.libiconv];
    Iso = [pkgs.libiconv];
    RandomFieldsUtils = [pkgs.libiconv];
    VGAM = [pkgs.libiconv];
    ahaz = [pkgs.libiconv];
    aws = [pkgs.libiconv];
    classInt = [pkgs.libiconv];
    clues = [pkgs.libiconv];
    cmprsk = [pkgs.libiconv];
    deldir = [pkgs.libiconv];
    dotCall64 = [pkgs.libiconv];
    drgee = [pkgs.libiconv];
    etm = [pkgs.libiconv];
    fields = [pkgs.libiconv];
    forecast = [pkgs.libiconv];
    fracdiff = [pkgs.libiconv];
    genefilter = [pkgs.libiconv];
    gstat = [pkgs.libiconv];
    hexbin = [pkgs.libiconv];
    lars = [pkgs.libiconv];
    leaps = [pkgs.libiconv];
    limSolve = [pkgs.libiconv];
    lmom = [pkgs.libiconv];
    lmtest = [pkgs.libiconv];
    matchingMarkets = [pkgs.libiconv];
    mixexp = [pkgs.libiconv];
    msm = [pkgs.libiconv];
    nleqslv = [pkgs.libiconv];
    pcaPP = [pkgs.libiconv];
    plgp = [pkgs.libiconv];
    preprocessCore = [pkgs.libiconv];
    robust = [pkgs.libiconv];
    rrcov = [pkgs.libiconv];
    signal = [pkgs.libiconv];
    spam = [pkgs.libiconv];
    strucchange = [pkgs.libiconv];
    tgp = [pkgs.libiconv];
    tmvtnorm = [pkgs.libiconv];
    tseries = [pkgs.libiconv];
    unmarked = [pkgs.libiconv];
    urca = [pkgs.libiconv];
    waveslim = [pkgs.libiconv];
